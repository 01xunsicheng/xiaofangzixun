...
lxb.add("config", function(a) {
	var c = lxb.use("base");
	var e = {
		BDCBID: '09ee15f6-8e8b-4278-a9ee-8182c8dad0a2',
		LXBVT: 0,
		TEMPSITEID: '10971292',
...
lxb.add("util", function(e) {
	...
	e.getFloat = function() {
		return {
			container: c.floatModule,
			closeBtn: c.closeModule
		}
	};
	...
	e.checkFloat = function() {
		if(document.readyState === "complete") {
			...
			r.create("img", {
				src: lxb.use("config").Root + "/count.gif?t=n&s=" + (o || q || 0)
			})
		}
	}
});
...
lxb.add("business.callback", function(b) {
	...
	f.prototype = {
		...
		_callCount: function() {
			a.create("img", {
				src: lxb.use("config").Root + "/count.gif?t=n"
			})
		},
		_doCall: function(k) {
			...
			if(!h(m)) {
				l.tip.show("error", {
					msg: e.ERROR_CB_PHONE
				});
				return
			}
			if(c.isPin()) {
				l.tip.show("pin", {
					callback: l.bind(l._submit, l),
					val: m,
					gid: k,
					timer: l.bind(l._setOpTimer, l)
				})
			} else {
				l._submit(m, k)
			}
		...
		_submit: function(p, n) {
			...
			var q = {
				vtel: p,
				siteid: o.cbInfo.siteId,
				bdcbid: k,
				refer_domain: c.getDomain(document.referrer) || ""
			};
			if(n) {
				q.g = n
			}
